...
/**
  ...
  */
class ScRateStreamProvider extends SimpleTableProvider with DataSourceRegister {

  import ScRateStreamProvider._
  ...
  override def shortName(): String = "rate-sc"
}
...
class RateStreamTable(
  ...
  override def schema(): StructType = ScRateStreamProvider.SCHEMA
  ...
  override def newScanBuilder(options: CaseInsensitiveStringMap): ScanBuilder = () => new Scan {
    override def readSchema(): StructType = ScRateStreamProvider.SCHEMA

    override def toMicroBatchStream(checkpointLocation: String): MicroBatchStream =
      new ScRateStreamMicroBatchStream(
        rowsPerSecond, rampUpTimeSeconds, numPartitions, options, checkpointLocation)
  }
}

object ScRateStreamProvider {
